...
/**
 * Creating a class we use List<String> as a parameter. Based on it we get hashMap
...
public class GraphService {
    ...
    public GraphService(List<String> points) {
        String sLog = "";
        for (int j = 0; j < points.size(); j++) {
            String[] sPoint = points.get(j).split(",");

            sLog += "{" + sPoint[0] + " ";
            sLog += sPoint[1] + "}";

            Node node = new Node();
            node.setIndex(0, Integer.parseInt(sPoint[0]));

            if (mapParentChildPairs.containsKey(Integer.parseInt(sPoint[1]))) {
                Node nodeCopy = mapParentChildPairs.get(Integer.parseInt(sPoint[1]));
                nodeCopy.setIndex(1, Integer.parseInt(sPoint[0]));
                mapParentChildPairs.put(Integer.parseInt(sPoint[1]), nodeCopy);
            } else mapParentChildPairs.put(Integer.parseInt(sPoint[1]), node);
        }
        log.info("inArr: " + sLog);
